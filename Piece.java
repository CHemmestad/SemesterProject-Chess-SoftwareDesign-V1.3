...
public class Piece {
    ...
    public void move(int xPosition, int yPosition) {
        ...
        System.out.println("Set to false"); //CHECK*****************
        Chess.check = false;
        for(int y = 0; y < 8; y++) {
            for( int x = 0; x < 8; x++) {
                Piece current = Chess.getPiece(x*100, y*100);
                if(current != null) {
                    current.inCheck = false;
                    current.moveAvailable = false;
                }
            }
        }
        firstMove = false;
    ...
    public void pawnAtEnd(Piece piece) {
        if(piece.name == "pawn") {
            if(piece.isWhite) {
                if(piece.yPosition == 0) {
                    piece.name = "queen";
                }
            } else {
    ...
    public int[][] checkKing() {
        System.out.println("Check King"); //CHECK*****************
        initialize(this.validMoves);
        if(name == "pawn") {
            moveCheckPawn(xPosition, yPosition, isWhite);
        } else if (name == "rook") {
            ...
            for(int x = 0; x < 8; x++) {
                if(validMoves[x][y] == 2) {
                    if((Chess.getPiece(x*100, y*100).name == "king") && (Chess.getPiece(x*100, y*100) != this)) {
                        ...
                        System.out.println(Chess.getPiece(x*100, y*100).name); //CHECK*****************
                        System.out.println("Found king"); //CHECK*****************
                        temp[x][y] = 3;
                        Piece tempP = Chess.getPiece(x*100, y*100);
                        tempP.inCheck = true;
                        System.out.println("In Check = true"); //CHECK*****************
                        tempP.moveAvailable = blockAvailable(this);
                        tempP.validMoves();
                    }
                }
            }
    ...
    public boolean blockAvailable(Piece checkedBy) { 
        boolean block = false;
        checkedBy.validMoves();
        for(int y = 0; y < 8; y++) {
            for(int x = 0; x < 8; x++) {
                Piece current = Chess.getPiece(x*100, y*100);
                if((current != null) && (current.isWhite != checkedBy.isWhite)) {
                    if(current.name != "king") {
                        current.validMoves();
                        for(int y2 = 0; y2 < 8; y2++) {
                            for(int x2 = 0; x2 < 8; x2++) {
                                if((current.validMoves[x2][y2] == -1) && (checkedBy.validMoves[x2][y2] == -1)) {
                                    current.validMoves[x2][y2] = -1;
                                    block = true;
                                } else if((current.validMoves[x2][y2] == 2) && ((x2 == checkedBy.xPosition) && (y2 == checkedBy.yPosition))){
                                    current.validMoves[x2][y2] = 2;
                                    block = true;
                                } else {
                                    current.validMoves[x2][y2] = 0;
                                }
                            }
                        }
                    }
    ...
    public void moveCheckPawn(int xp, int yp, boolean isWhite) {
        System.out.println("Movecheck pawn"); //CHECK*****************
        if(isWhite) {
            if(firstMove) {
